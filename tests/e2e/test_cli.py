...
from pyscrobbler.config import Config
from pyscrobbler.entrypoints.cli import cli
from pyscrobbler.version import __version__
...
def fixture_runner(config: Config) -> CliRunner:
    ...
    return CliRunner(
        mix_stderr=False, env={"PYSCROBBLER_CONFIG_PATH": config.config_path}
    )
...
def test_version(runner: CliRunner) -> None:
    ...
    assert re.match(
        fr" *pyscrobbler version: {__version__}\n"
        r" *python version: .*\n *platform: .*",
        result.stdout,
    )
